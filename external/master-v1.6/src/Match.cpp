...
{
	Match* m = (Match *)&other;
	initMatch(m->getSearchResults(), m->getTsIdx(), m->getKeepOrder());
...
void Match::initMatch(SearchResults* p, const int & idx, const bool & ko)
{
	setKeepOrder(ko);
	setTsIdx(idx);
	setSearchResults(p);
...
{
	if ((!_keeporder) && (_rmsd != other.getRmsd()))
	{
...
int Match::parseMatch(char* line, const vector<vector<int> > & gaplen)
{
	...
	{
		ASSERT(sscanf(cur, "%lf", &_rmsd) == 1, "could not parse match from line, current state: %s", line);
	}
	...
	while (*(cur + strlen(cur) + 1) != ']')
	{
		...
		{
			ASSERT(sscanf(cur, "%d", &(_bres[i])) == 1, "could not parse match from line, current state: %s", line);
		...
		if (i < _sr->getQsNumSeg())
		{
			ASSERT(sscanf(cur, "%d", &eres) == 1, "could not parse match from line, current state: %s", line);
		}
...
bool cmpMatchPairByTarget(pair<Match*, int> a, pair<Match*, int> b)
{
	return (a.first->getTsIdx() < b.first->getTsIdx());
}
