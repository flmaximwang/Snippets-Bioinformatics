...
class Match
{
	public:
		...
		Match(SearchResults* p = NULL, const int & idx = -1, const bool & ko = false) { initMatch(p, idx, ko); }
		~Match();

		void delBegRes() { delete [] _bres; }
		void delRotation();
		void delTranslation() { delete [] _t; }
		...
		void initMatch(SearchResults* p = NULL, const int & idx = -1, const bool & ko = false);
	...
	protected:		
		...
		bool _keeporder; // whether or not to keep the insertion order of matches
...
bool cmpMatchPairByRmsd(pair<Match*, int>, pair<Match*, int>);
bool cmpMatchPairByTarget(pair<Match*, int>, pair<Match*, int>);
