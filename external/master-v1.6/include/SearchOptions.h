...
class SearchOptions
{
	public:		
		...
		bool getSkipRMSD() const { return _skiprmsd; }
		...
		void setSkipRMSD(const bool & f) { _skiprmsd = f; }
	...
	protected:
		...
		bool _keeporder; // whether or not to keep the insertion order of matches when specifying '--matchIn'
		string _mifname; // input match list file
		int _minn; // find at least this many hits during search
		...
		int _rmode; // RMSD bounding mode
		double _rthresh; // RMSD cutoff
		bool _skiprmsd; // whether or not to skip RMSD re-calculation when specifying '--matchIn'
		...
		double _tune; // tuning parameter for RMSD bounding
};
